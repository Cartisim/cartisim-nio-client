...
public class NIOTSHandler {
    ...
    internal init(host: String, port: Int, isEncryptedObject: Bool, tls: Bool, groupProvider provider: EventLoopGroupManager.Provider) {
        self.host = host
        self.port = port
...
//    }
    ...
    internal func sendEncryptedObject<T>(chat: T) {
        channel?.writeAndFlush(chat, promise: nil)
        dataReceived()
    }
